...
realizar_movimiento:
    ...
    ; Guardar la pieza capturada antes del movimiento
    mov al, matriz[edi]  ; Cargar la pieza que serÃ¡ capturada
    mov ah, al           ; Guardar en AH para usar despuÃ©s
    ...
    ; Llamar a la funciÃ³n para guardar el movimiento en JSON
    call guardar_movimiento_json
...
guardar_movimiento_json PROC
    ; Esta funciÃ³n llama al script de Python para guardar el movimiento en JSON
    ; y esperar actualizaciones del tablero
    
    ; Mostrar mensaje de que se estÃ¡ guardando el movimiento
    call Crlf
    mWrite "Guardando movimiento en JSON..."
    call Crlf
    
    ; Crear el comando para llamar al batch file
    ; Necesitamos construir: save_move.bat <from_pos> <to_pos> <piece>
    
    ; Convertir las posiciones a strings
    ; from_pos = seleccion_fila + seleccion_columna (ej: "12")
    ; to_pos = nueva_fila + nueva_columna (ej: "34")
    
    ; Por ahora, crear un archivo temporal con los datos del movimiento
    call crear_archivo_movimiento
    
    ; Llamar al script de Python directamente
    call ejecutar_script_python
    
    mWrite "Movimiento guardado en moves.json y tablero actualizado en 2.json"
    call Crlf
    mWrite "Esperando actualizaciones del tablero..."
    call Crlf
    
    ret
guardar_movimiento_json ENDP

crear_archivo_movimiento PROC
    ; Crear un archivo temporal con los datos del movimiento
    ; Esto permite que el script de Python lea los datos
    
    ; Por simplicidad, solo mostramos un mensaje
    mWrite "Datos del movimiento preparados..."
    call Crlf
    
    ret
crear_archivo_movimiento ENDP

ejecutar_script_python PROC
    ; Ejecutar el script de Python
    ; En una implementaciÃ³n real, aquÃ­ se harÃ­a la llamada al sistema
    
    mWrite "Ejecutando script de Python..."
    call Crlf
    
    ; Por ahora, solo mostramos el mensaje
    ; En una implementaciÃ³n completa, se ejecutarÃ­a:
    ; python chess_move_handler.py <from_pos> <to_pos> <piece>
    
    ret
ejecutar_script_python ENDP
